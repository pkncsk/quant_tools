...
WARMUP_BARS = 250
BAR_INTERVAL_MIN = 5   # default interval in fetch_fx
...
params = [(f, s, slp, r) for f in FAST_RANGE for s in SLOW_RANGE
          for slp in SL_LIST for r in RISK_LIST]
...
def build_entry(idx, fast_sma, slow_sma, trend_sma, risk_pct, sl_pips, name):
    class _EntrySlope:
        ...
        def check(self, ts, price_val, _df):
            i = idx.get_loc(ts)
            # require both warm-up and official start date
            if ts < START or i < WARMUP_BARS:
                return (False, {})
...
@ray.remote
class FullWorker:
    def __init__(self, df_ref, pair_params, fees_params,
        ...
        winset = sorted({int(x) for x in (fast_vals or [])}
                        | {int(x) for x in (slow_vals or [])} | {200})
    ...
    def run(self, f, s, slp, r):
        ...
        if trend_sma is None:
            trend_sma = self.df["close"].rolling(200).mean(); self._sma[200] = trend_sma
...
class FastWorker:
    def __init__(self, df_ref, pair_params, fees_params,
                 initial_capital, fast_vals=None, slow_vals=None):
        from quant_tools import CurrencyPair, Fees
        import ray as _ray
        try: from ray import ObjectRef
        except Exception: ObjectRef = ()
...
if Worker is FullWorker:
    actors = [Worker.remote(df_ref, pair_params, fees_params,
                            SPREAD_PIPS, SLIPPAGE_PIPS, INITIAL_CAPITAL,
...
print(df_results_ray.head(12))
#%%
df_results_ray.to_csv("./test_sweep_sma.txt", sep='\t')
# %%
df_results_ray
